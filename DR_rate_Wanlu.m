...
[Am,qcharge,Eion,Ie,radius_e,start,stop] = deal(78,19,3.85,0.10017,2.7,2,8);
%第1列DCCT，第2列Counts，默认第3列NIM3，第5列NIM1。根据实际NIM信号更改*位置。
data_format = '%f %f %f %f %*f %*f %*f %*f %*f %*f';   %*f表示丢弃这一列的数据。


normalizaed = 0;                  %是否用冷却点归一化调制点
v_scale = 1;                   % 电压校正因子，这个值可以根据情况修改；
v_pos_neg = [1 -1];
...
if isequal(pathname,0)
    plot_collision_energy(detuning_voltages,Erel,Am, qcharge,Eion,Ie);
...
Alpha_c = zeros(size(count_over_dcct));
sum_counts = zeros(size(count_over_dcct));
...
function current_path = default_folder(is_default)
    ...
    if is_default
        %start_path = ['C:\Users\',getenv('username'),'\Desktop']; %默认从桌面打开，也可以直接更改默认的起始路径
        start_path = 'D:\01-Works\DataAnalysis\78Kr19+'; % 路径更改为当前数据文件夹，方便操作
        cd(start_path)
    end
end
function plot_collision_energy(detuning_voltages,Erel,Am, qcharge,Eion,Ie)
